...
localStorage.setItem('cube_username', 'tahubulat', 'agung','pepek');
...
function showSection(sectionId) {
    // 1. Sembunyikan semua section dengan aman
    ...
    // ==========================================================================
    // SINKRONISASI PENGHENTI VIDEO: MATIKAN ANIME JIKA PINDAH KE MENU LAIN
    // ==========================================================================
    if (sectionId !== 'cube-anime') {
        // Hentikan video YouTube di latar belakang agar suaranya mati
        if (typeof animePlayer !== 'undefined' && animePlayer && typeof animePlayer.pauseVideo === 'function') {
            animePlayer.pauseVideo();
        }
        // Matikan hitungan durasi bar menit video
        if (typeof animeProgressInterval !== 'undefined') {
            clearInterval(animeProgressInterval);
        }
        // Sembunyikan bingkai pemutar videonya agar tidak membeku di layar
    ...
    // 5. Update saldo jika membuka wallet
    ...
    // Di dalam fungsi showSection milikmu, tambahkan kode ini:
    if (sectionId === 'dashboard') {
        checkAdminOtoritasDashboard();
    }

    // 7. Geser layar kembali ke posisi paling atas
...
// GUNAKAN API KEY GROQ KAMU DI SINI
  const GROQ_API_KEY = "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"; 
  ...
  async function sendAiMessage() {
    ...
    // 1. Tampilkan Pesan User (Diperbarui agar bisa menampilkan gambar jika dilampirkan)
    let userBubbleHtml = `
        <div style="background: rgba(255,255,255,0.1); padding: 12px 18px; border-radius: 18px 18px 0 18px; align-self: flex-end; max-width: 80%; border: 1px solid rgba(255,255,255,0.1); font-family: 'Rajdhani'; margin-bottom: 12px; color: #fff; box-shadow: 0 4px 15px rgba(0,0,0,0.2); display: flex; flex-direction: column; gap: 8px;">
    `;
    
    if (imagePayload) {
        userBubbleHtml += `<img src="${imagePayload}" style="max-width: 100%; max-height: 180px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.15);" alt="Preview">`;
    }
    if (userMsg) {
        userBubbleHtml += `<span>${userMsg}</span>`;
    }
    userBubbleHtml += `</div>`;
    ...
    chatBox.innerHTML += `
        <div id="${typingId}" style="color: #0f0; font-size: 0.8rem; font-family: 'Orbitron'; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
            <i class="fa-solid fa-microchip fa-spin"></i> CUBE AI IS PROCESSING...
        </div>
    `;
...
// Fungsi Sinkronisasi Mendengar Seluruh Katalog Produk yang Ada di Pasar Digital via db2
function listenStoreCatalog() {
    db2.ref("cube_store/products").on("value", function(snapshot) {
        ...
        if(!snapshot.exists()) {
            grid.innerHTML = `<p style="grid-column: 1/-1; text-align:center; opacity:0.4; font-size:0.9rem; padding:40px 0;">Belum ada akun game atau produk digital yang dipasang iklan.</p>`;
            return;
        }
        ...
        snapshot.forEach(child => {
            const id = child.key;
            const data = child.val();
            liveProductStoreCache[id] = data; // Amankan ke memori lokal cache aplikasi

            // Evaluasi Tampilan Harga Coret Promo
            const textStrike = data.strike ? `<span class="store-prod-price-strike">Rp ${parseInt(data.strike).toLocaleString('id-ID')}</span>` : '';

            grid.innerHTML += `
                <div class="store-prod-card">
                    <div class="store-prod-img-wrapper" style="background-image: url('${data.photo}');">
                        <span class="store-prod-badge-seller"><i class="fa-solid fa-user-shield"></i> ${data.seller.toUpperCase()}</span>
                    </div>
                    <div class="store-prod-content">
                        <h4 class="store-prod-title-text">${data.title}</h4>
                        <div class="store-prod-price-row">
                            <span class="store-prod-price-main">Rp ${parseInt(data.price).toLocaleString('id-ID')}</span>
                            ${textStrike}
                        </div>
                        <p class="store-prod-desc-text">${data.desc}</p>
                        <button class="btn-store-buy" onclick="checkoutProductStore('${id}')"><i class="fa-solid fa-bag-shopping"></i> BELI PRODUK</button>
                    </div>
                </div>
            `;
        });
                        ...
                        if (db2 && currentUser) {
                            db2.ref("cube_users/" + currentUser + "/balance").once("value", function(snap) {
                                if(snap.exists()) {
                                    ...
                                    // 3. Update element saldo spesifik yang ada di CUBEQRIS Hub
                                    const hubBalanceView = document.getElementById('hub-live-balance');
                                    if (hubBalanceView) {
                                        hubBalanceView.innerText = "Rp " + latestBalance.toLocaleString('id-ID');
                                    }
                                }
                            });
                        }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateBatteryData(battery) {
        ...
        if (battery.charging) {
            document.getElementById('bat-status').innerText = "// POWER_CONNECTED_CHARGING";
            batIconEl.className = "fa-solid fa-battery-bolt fa-bounce";
        } else {
            document.getElementById('bat-status').innerText = `// DISCHARGING_SYS_HEALTH_OK`;
    ...
    setInterval(() => {
        ...
        let tempDesc = "// THERMAL_STABLE_GREEN";
        
        if (finalTemp > 39.5) tempDesc = "// WARNING_THERMAL_HIGH";
