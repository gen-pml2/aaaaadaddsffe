...
/* ==========================================================================
   CUBE STABILITY PATCH - added by Codex
   Keeps navigation and common helpers from crashing when optional services lag.
   ========================================================================== */
(function () {
    function cubeGetDb2() {
        try {
            return (typeof db2 !== "undefined" && db2 && typeof db2.ref === "function") ? db2 : null;
        } catch (error) {
            return null;
        }
    }

    function cubeAlertError(title, message) {
        if (typeof ErenAlert !== "undefined" && ErenAlert && typeof ErenAlert.error === "function") {
            ErenAlert.error(title, message);
            return;
        }
        alert(title + "\n" + message);
    }
    ...
    if (typeof originalCheckLogin === "function") {
        window.checkLogin = function () {
            if (!cubeGetDb2()) {
                cubeAlertError("Database Belum Siap", "Koneksi akun sedang belum siap. Tunggu beberapa detik lalu coba lagi.");
                return;
            }
            return originalCheckLogin.apply(this, arguments);
        };
    }

    const originalHandleRegister = window.handleRegister;
    if (typeof originalHandleRegister === "function") {
        window.handleRegister = function () {
            if (!cubeGetDb2()) {
                cubeAlertError("Database Belum Siap", "Koneksi registrasi sedang belum siap. Tunggu beberapa detik lalu coba lagi.");
                return;
            }
    ...
    window.showSection = function (sectionId) {
        ...
        if (sectionId === "admin-lobby") {
            if (typeof Swal === "undefined") {
                cubeAlertError("Library Belum Siap", "Komponen popup belum termuat. Coba ulangi sebentar lagi.");
                return;
            }

            if (typeof checkNotifStatus === "function") checkNotifStatus();
            ...
            Swal.fire({
                ...
                if (!result.isConfirmed) return;
                if (result.value === "cemplok") {
                    if (typeof ErenAlert !== "undefined") ErenAlert.success("ACCESS GRANTED", "Otoritas diverifikasi.");
                    if (typeof previousSectionId !== "undefined") previousSectionId = currentSectionId;
                    if (typeof currentSectionId !== "undefined") currentSectionId = sectionId;
                    if (typeof executeSectionRender === "function") executeSectionRender("admin-lobby");
                } else {
                    cubeAlertError("ACCESS DENIED", "Kunci admin salah.");
                }
            });
            return;
        }

        if (typeof previousSectionId !== "undefined" && typeof currentSectionId !== "undefined" && sectionId !== currentSectionId) {
            previousSectionId = currentSectionId;
            currentSectionId = sectionId;
        }

        if (typeof executeSectionRender === "function") {
            executeSectionRender(sectionId);
        }

        if (sectionId === "message-chat" && typeof scrollToBottomChat === "function") {
            setTimeout(scrollToBottomChat, 200);
        }

        if (sectionId === "analytics-view" && typeof scrollToLatestAnalytics === "function") {
            scrollToLatestAnalytics();
        }
    };
...
/* ==========================================================================
   CUBE QRIS TRANSFER ENGINE - added by Codex
   ========================================================================== */
(function () {
    function qrisActiveUser() {
        return ((typeof currentUser !== "undefined" && currentUser) || localStorage.getItem("activeUser") || localStorage.getItem("cube_username") || "").trim().toLowerCase();
    }
    ...
    function qrisCleanUser(rawText) {
        ...
        try {
            if (/^https?:\/\//i.test(text)) {
                ...
                text = url.searchParams.get("user") || url.searchParams.get("username") || url.searchParams.get("to") || text;
            }
        } catch (error) {}

        text = text.replace(/^CUBEPAY_ID:/i, "");
        text = text.replace(/^cubepay_id:/i, "");
        text = text.replace(/^@/, "");
        return text.trim().toLowerCase().replace(/[^a-z0-9._-]/g, "");
    }

    function qrisSetTarget(rawText) {
        const username = qrisCleanUser(rawText);
        const targetInput = document.getElementById("hub-tf-target");
        const amountInput = document.getElementById("hub-tf-amount");
        if (!username) {
            if (typeof ErenAlert !== "undefined") ErenAlert.error("QRIS Tidak Valid", "Username tujuan tidak terbaca dari QRIS.");
            return;
        }
        ...
        if (typeof Swal !== "undefined") {
            Swal.fire({
                icon: "success",
                title: "QRIS Teman Terbaca",
                text: "Tujuan transfer: @" + username.toUpperCase() + ". Masukkan nominal lalu kirim saldo.",
                background: "#030303",
                color: "#fff",
                confirmButtonColor: "#22d3ee",
                didClose: function () { if (amountInput) amountInput.focus(); }
            });
        }
    }

    window.processScannedQrisData = function (textData) {
        qrisSetTarget(textData);
    };

    window.handleHubQRISDecode = function (event) {
        const file = event && event.target && event.target.files ? event.target.files[0] : null;
        if (!file) return;

        const done = function () { if (event.target) event.target.value = ""; };

        if (typeof Html5Qrcode !== "undefined") {
            let scanner;
            try {
                scanner = new Html5Qrcode("qris-camera-reader");
                scanner.scanFile(file, true).then(function (decodedText) {
                    qrisSetTarget(decodedText);
                    done();
                }).catch(function () {
                    decodeWithCanvas(file, done);
                });
            } catch (error) {
                decodeWithCanvas(file, done);
            }
    ...
    function decodeWithCanvas(file, done) {
        if (typeof jsQR === "undefined") {
            if (typeof ErenAlert !== "undefined") ErenAlert.error("Scanner Belum Siap", "Library pembaca QR belum termuat. Coba ulangi sebentar lagi.");
            if (done) done();
            return;
        }
        ...
        reader.onload = function (e) {
            ...
            img.onload = function () {
                ...
                if (code && code.data) qrisSetTarget(code.data);
                else if (typeof ErenAlert !== "undefined") ErenAlert.error("Scan Gagal", "QRIS pada gambar belum terbaca. Coba pakai foto yang lebih jelas.");
                if (done) done();
            };
            img.src = e.target.result;
        };
    ...
    window.startLiveQrisScanner = function () {
        const container = document.getElementById("live-scanner-container");
        if (!container) return;
        if (typeof Html5Qrcode === "undefined") {
            if (typeof ErenAlert !== "undefined") ErenAlert.error("Kamera Belum Siap", "Library scanner belum termuat. Coba ulangi sebentar lagi.");
            return;
        }

        container.style.display = "block";
        if (window.liveQrisScannerInstance && typeof window.liveQrisScannerInstance.stop === "function") {
            window.stopLiveQrisScanner();
        }

        window.liveQrisScannerInstance = new Html5Qrcode("qris-camera-reader");
        window.liveQrisScannerInstance.start(
            { facingMode: "environment" },
            ...
            console.error(error);
            container.style.display = "none";
            if (typeof ErenAlert !== "undefined") ErenAlert.error("Kamera Ditolak", "Izinkan akses kamera untuk scan QRIS realtime.");
        });
    };

    window.stopLiveQrisScanner = function () {
        const container = document.getElementById("live-scanner-container");
        const scanner = window.liveQrisScannerInstance || (typeof liveQrisScannerInstance !== "undefined" ? liveQrisScannerInstance : null);
        if (scanner && typeof scanner.stop === "function") {
            scanner.stop().catch(function () {}).finally(function () {
                if (container) container.style.display = "none";
                window.liveQrisScannerInstance = null;
                try { liveQrisScannerInstance = null; } catch (error) {}
            });
        } else if (container) {
            container.style.display = "none";
        }
    ...
    window.executeHubTransferSaldo = async function () {
        const senderUser = qrisActiveUser();
        const targetUser = qrisCleanUser(document.getElementById("hub-tf-target")?.value);
        const amount = parseInt(document.getElementById("hub-tf-amount")?.value || "0", 10);
        const database = qrisDb();

        if (!senderUser) return ErenAlert.error("Sesi Habis", "Silakan login kembali sebelum transfer.");
        if (!database) return ErenAlert.error("Database Belum Siap", "Koneksi saldo belum siap. Coba lagi sebentar.");
        if (!targetUser) return ErenAlert.error("Tujuan Kosong", "Masukkan username tujuan atau scan/upload QRIS teman.");
        if (senderUser === targetUser) return ErenAlert.error("Tujuan Sama", "Transfer ke akun sendiri tidak diperlukan. Gunakan username teman sebagai penerima.");
        if (isNaN(amount) || amount < 1) return ErenAlert.error("Nominal Salah", "Nominal transfer minimal Rp 1.");

        if (typeof ErenAlert !== "undefined") ErenAlert.loading("Memproses transfer saldo...");

        try {
            const senderRef = database.ref("cube_users/" + senderUser);
            const receiverRef = database.ref("cube_users/" + targetUser);
            const senderSnap = await senderRef.once("value");
            const receiverSnap = await receiverRef.once("value");

            if (!senderSnap.exists()) throw new Error("Akun pengirim tidak ditemukan.");
            if (!receiverSnap.exists()) throw new Error("Username @" + targetUser + " tidak ditemukan.");

            const senderBalance = parseInt(senderSnap.val().balance || 0, 10);
            const receiverBalance = parseInt(receiverSnap.val().balance || 0, 10);
            if (senderBalance < amount) throw new Error("Saldo Anda Rp " + senderBalance.toLocaleString("id-ID") + ", belum cukup untuk transfer ini.");
            ...
            updates["cube_users/" + senderUser + "/balance"] = newSenderBalance;
            updates["cube_users/" + targetUser + "/balance"] = newReceiverBalance;
            updates["cube_users/" + senderUser + "/history/" + txId] = { type: "transfer_out", target: targetUser, amount: amount, balanceAfter: newSenderBalance, time: Date.now() };
            updates["cube_users/" + targetUser + "/history/" + txId] = { type: "transfer_in", from: senderUser, amount: amount, balanceAfter: newReceiverBalance, time: Date.now() };
            updates["cube_transactions/" + txId] = { tx_id: txId, type: "Transfer Saldo", sender: senderUser, receiver: targetUser, amount: amount, timestamp: new Date().toISOString() };
            updates["cube_notifications/" + targetUser + "/" + txId] = { title: "Transfer Masuk", message: "Anda menerima saldo Rp " + amount.toLocaleString("id-ID") + " dari @" + senderUser.toUpperCase(), type: "transfer_in", isRead: false, time: Date.now() };

            await database.ref().update(updates);

            localStorage.setItem("cube_balance", newSenderBalance);
            if (typeof updateBalanceUI === "function") updateBalanceUI(newSenderBalance);
            const hubBalance = document.getElementById("hub-live-balance");
            const sidebarBalance = document.getElementById("sidebar-user-balance");
            if (hubBalance) hubBalance.innerText = "Rp " + newSenderBalance.toLocaleString("id-ID");
            if (sidebarBalance) sidebarBalance.innerText = "Rp " + newSenderBalance.toLocaleString("id-ID");

            if (typeof Swal !== "undefined") Swal.close();
            ErenAlert.success("Transfer Berhasil", "Saldo -Rp " + amount.toLocaleString("id-ID") + " berhasil dikirim ke @" + targetUser.toUpperCase() + ".");
            document.getElementById("hub-tf-target").value = "";
            document.getElementById("hub-tf-amount").value = "";
        } catch (error) {
            if (typeof Swal !== "undefined") Swal.close();
            console.error(error);
            ErenAlert.error("Transfer Gagal", error.message || "Gagal memproses transfer saldo.");
        }
    };

    const originalGenerateMyInternalQRIS = window.generateMyInternalQRIS;
    window.generateMyInternalQRIS = function () {
        const qrContainer = document.getElementById("hub-static-qrcode") || document.getElementById("qrcode-internal-area");
        const username = qrisActiveUser();
        if (!qrContainer || !username || typeof QRCode === "undefined") {
            if (typeof originalGenerateMyInternalQRIS === "function") return originalGenerateMyInternalQRIS.apply(this, arguments);
            return;
        }
        qrContainer.innerHTML = "";
        new QRCode(qrContainer, {
            text: "CUBEPAY_ID:" + username,
            width: 180,
            height: 180,
            colorDark: "#000000",
            colorLight: "#ffffff",
        ...
        const owner = document.getElementById("hub-qris-owner-name") || document.getElementById("qris-username-lbl");
        if (owner) owner.innerText = "@" + username.toUpperCase();
    };
})();

/* ==========================================================================
   CUBE QRIS TAB FINALIZER - added by Codex
   ========================================================================== */
(function () {
    function showOnlyQrisPanel(tabId) {
        ["tab-view-transfer", "tab-view-topup", "tab-view-tukar", "tab-view-mutasi"].forEach(function (id) {
            const panel = document.getElementById(id);
            if (panel) panel.style.display = (id === tabId) ? "block" : "none";
        });

        const qrisCard = document.getElementById("cube-qris-card");
        if (qrisCard) qrisCard.style.display = (tabId === "tab-view-qris") ? "block" : "none";

        ["tab-view-qris", "tab-view-transfer", "tab-view-topup", "tab-view-tukar", "tab-view-mutasi"].forEach(function (id) {
            const btn = document.getElementById("btn-" + id);
            if (btn) btn.classList.toggle("active", id === tabId);
        });

        if (tabId === "tab-view-qris" && typeof generateMyInternalQRIS === "function") {
            generateMyInternalQRIS();
        }
    }

    window.switchQrisTab = function (tabId) {
        showOnlyQrisPanel(tabId || "tab-view-qris");
    };

    const previousShowSection = window.showSection;
    window.showSection = function (sectionId) {
        if (typeof previousShowSection === "function") previousShowSection.apply(this, arguments);
        if (sectionId === "cubeqris-hub") {
            setTimeout(function () {
                if (typeof switchQrisTab === "function") switchQrisTab("tab-view-qris");
                if (typeof generateMyInternalQRIS === "function") generateMyInternalQRIS();
            }, 120);
        }
    };
})();
